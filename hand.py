...
import math
import cv2
from getHSV import get_hsv

# video capture
cap = cv2.VideoCapture(0)
lower_bound, upper_bound = get_hsv(cap)
...
while cap.isOpened():
    ...
    lower_skin = np.array(lower_bound, dtype=np.uint8)
    upper_skin = np.array(upper_bound, dtype=np.uint8)
